...
module.exports = {
    customId: 'gamesclearrole_button',
    /**
     * 
    ...
    run: async (client, interaction) => {
        try {
            ...
            const embed = new EmbedBuilder()
                .setColor('Random')
                .setDescription(`<a:clearemojie:1188137273271799909> <@${interaction.user.id}>: All games roles removed successfully!`);

            await interaction.reply({ embeds: [embed], ephemeral: true, flags: [4096] });
        } catch (error) {
            console.error('Error handling gamesclearrole_button click:', error);
