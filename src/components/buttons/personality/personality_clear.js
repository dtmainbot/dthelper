...
module.exports = {
    customId: 'personalityclearrole_button',
    /**
     * 
    ...
    run: async (client, interaction) => {
        try {
            ...
            const embed = new EmbedBuilder()
                .setColor('Random')
                .setDescription(`<a:clearemojie:1188137273271799909> <@${interaction.user.id}>: All personality roles removed successfully!`);

            await interaction.reply({ embeds: [embed], ephemeral: true, flags: [4096] });
        } catch (error) {
            console.error('Error handling personalityclearrole_button click:', error);
