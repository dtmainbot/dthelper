    ...
    run: async (client, interaction) => {
        try {
            ...
            const embed = new EmbedBuilder()
                .setColor('Random')
                .setDescription(`<a:clearemojie:1188137273271799909> <@${interaction.user.id}>: All age roles removed successfully!`);

            await interaction.reply({ embeds: [embed], ephemeral: true, flags: [4096] });
        } catch (error) {
            console.error('Error handling ageclearrole_button click:', error);
