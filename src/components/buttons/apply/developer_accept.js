...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        if (hasManageRolesPermission || hasManagerRole) {
            ...
            const developerRole = helperApplyData.developerRole;

            // Assign the developerRole to the user
            const role = interaction.guild.roles.cache.find(role => role.id === developerRole);
            if (role) {
                ...
                if (applylogsChannel) {
                    const embed = new EmbedBuilder()
                        ...
                        <a:typeemojie:1188572572766453860> __**Apply Type**__:\n<:arrowflipet:1188472519213207582>\`Developer Application\`.\n
                        `)
                ...
                await interaction.reply({content: `✅ Developer role has been assigned to <@${userId}>..`, flags: [4096], ephemeral: true});
            } else {
                await interaction.reply({content: 'Error: Developer role not found.', flags: [4096], ephemeral: true});
            }
        } else {
