...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        // If the user has the necessary permissions, give them the designerRole
        if (hasManageRolesPermission || hasManagerRole) {
            ...
            const designerRole = helperApplyData.designerRole;

            // Assign the modRole to the user
            const role = interaction.guild.roles.cache.find(role => role.id === designerRole);
            if (role) {
                ...
                await interaction.reply({content: `✅ Designer role has been assigned to <@${userId}>..`, flags: [4096], ephemeral: true});
            } else {
                await interaction.reply({content: 'Error: Designer role not found.', flags: [4096], ephemeral: true});
            }
        } else {
